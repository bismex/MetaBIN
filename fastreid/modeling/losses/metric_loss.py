...
def euclidean_dist(x, y):
    ...
    # torch.addmm(input = dist, beta = 1, alpha = -2, mat1 = x, mat2 = y.t()) #
...
def hard_example_mining(dist_mat, is_pos, is_neg):
    ...
    try:
        ...
        # relative_p_inds = torch.Tensor()
        # dist_an = torch.Tensor()
        # relative_n_inds = torch.Tensor()
    except:
        for i in range(N):
            local_dist = dist_mat[i]
            local_dist_ap, local_relative_p_inds = torch.max(local_dist[is_pos[i]], 0)
            local_dist_an, local_relative_n_inds = torch.min(local_dist[is_neg[i]], 0)
            if i == 0:
                dist_ap = local_dist_ap.unsqueeze(0)
                dist_an = local_dist_an.unsqueeze(0)
                relative_p_inds = local_relative_p_inds.unsqueeze(0)
                relative_n_inds = local_relative_n_inds.unsqueeze(0)
            else:
                dist_ap = torch.cat((dist_ap, local_dist_ap.unsqueeze(0)), dim=0)
                dist_an = torch.cat((dist_an, local_dist_an.unsqueeze(0)), dim=0)
                relative_p_inds = torch.cat((relative_p_inds, local_relative_p_inds.unsqueeze(0)), dim=0)
                relative_n_inds = torch.cat((relative_n_inds, local_relative_n_inds.unsqueeze(0)), dim=0)
